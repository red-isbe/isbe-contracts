...
export class pkmanagement {
    ...
    generateK1(outputFile: string, prefundAmount: bigint) {
        console.log(`Generating K1 genesis file: ${outputFile} with prefund amount: ${prefundAmount}`);

        try {
            // Validate private keys array
            if (this.privateKeys.length === 0) {
                throw new Error('No private keys available. Please ensure the private key file was loaded correctly.');
            }
            ...
            for (let i = 0; i < this.privateKeys.length; i++) {
                ...
                try {
                    let address: string = computeAddress(pk);
                    r1Entries.push({ address, prefundAmount });
                } catch (error) {
                    throw new Error(`Failed to compute address for private key [${i}]: ${error.message}`);
                }
            }
            ...
            try {
                fs.writeFileSync(outputFile, JSON.stringify(genesisTemplate, null, 2), 'utf-8');
                console.log(`✅ K1 genesis file created successfully: ${outputFile}`);
                console.log(`   - Total addresses: ${r1Entries.length}`);
                console.log(`   - Prefund amount per address: ${prefundAmount.toString()} wei`);
            } catch (error) {
    ...
    async generateR1(outputFile: string, prefundAmount: bigint) {
        console.log(`Generating R1 genesis file: ${outputFile} with prefund amount: ${prefundAmount}`);
        ...
        try {
            ...
            for (let i = 0; i < this.privateKeys.length; i++) {
                ...
                try {
                    ...
                    if (!/^[0-9a-fA-F]+$/.test(cleanPrivateKey)) {
                        throw new Error('Private key contains invalid hexadecimal characters');
                    }
                    ...
                    throw new Error(`Failed to generate secp256r1 address for private key [${i}] (${keyPreview}): ${error.message}`);
                }
            }
            ...
            try {
                fs.writeFileSync(outputFile, JSON.stringify(genesisTemplate, null, 2), 'utf-8');
                console.log(`✅ R1 genesis file created successfully: ${outputFile}`);
                console.log(`   - Total addresses: ${r1Entries.length}`);
                console.log(`   - Prefund amount per address: ${prefundAmount.toString()} wei`);
            } catch (error) {
                throw new Error(`Failed to write output file ${outputFile}: ${error.message}\nPlease check write permissions and disk space.`);
