...
async function processTX(
    ...
    process.stdout.write(
        `\x1b[31m ${message} - Process transaction (TX: ${(await tx).hash})...\x1b[0m\r`
    )
    await pTx.wait()
    process.stdout.write(
        `${message} - Process transaction (TX: ${(await tx).hash}) \x1b[32m[OK]\x1b[0m              \n`
    )
}
...
async function validateRoles(
    ...
    await processTX(
        'Revoke Role',
        accessControlContract.revokeRole(role, signerAddress)
    )
