...
export class ContractRegistry {
  ...
  public retrieveContractRegistry(inputFile: string): void {
    ...
    try {
      ...
      throw new Error(`❌ Invalid JSON in ${inputPath}: ${(err as Error).message}`);
    }
  ...
  public getAddresses(contractName: string): string[] {
    ...
    if (!result) {
      throw new Error(`❌ Contract name ${contractName} not found in registry`);
    }
    return result;
  }
  ...
  public getAddress(contractName: string): string {
    ...
    if (!result || result.length !== 1) {
        throw new Error(`❌ Expected exactly one address for contract ${contractName}, found ${result ? result.length : 0}`);
    }
    return result[0];
  }
}