...
const EVENT_NAME = 'VerificationMethodExpired'

export interface VerificationMethodExpiredResult {
    did: string
    vMethodId: string
    notAfter: bigint
}
...
export async function expireVerificationMethod(
...
): Promise<VerificationMethodExpiredResult> {
    console.log(
        `⏰ Using ${signatureProvider.getCurveType()} signature for expiring verification method...`
    )

    const notAfterBigInt = BigInt(notAfter)

    if (signatureProvider.getCurveType() === 'secp256r1') {
        return await expireVerificationMethodWithRawTransaction(
            hre,
            did,
            vMethodId,
            notAfterBigInt,
    ...
    console.log('📡 Sending expireVerificationMethod transaction...')
    let tx: ContractTransactionResponse
    try {
        tx = await didVerificationMethodFacet.expireVerificationMethod(
            did,
            vMethodId,
            notAfterBigInt
        )
    ...
    if (
        ...
        throw new Error('Invalid VerificationMethodExpired event args format')
    }
    ...
    console.log(`\n✅ Verification method expiration set successfully:`)
...
/**
 * Expire verification method using raw transactions for secp256r1 compatibility
 * Avoids the "Cannot find square root" error by bypassing ethers Contract interface
 */
async function expireVerificationMethodWithRawTransaction(
    hre: HardhatRuntimeEnvironment,
    did: string,
    vMethodId: string,
    notAfter: bigint,
    ...
    // Encode the expireVerificationMethod function call
    const functionData = contractInterface.encodeFunctionData(
        'expireVerificationMethod',
        [did, vMethodId, notAfter]
    )

    console.log('📡 Sending expireVerificationMethod raw transaction...')
    ...
    try {
        ...
        txResponse = await signatureProvider.sendTransaction({
            to: diamond,
            data: functionData,
            gasLimit: 800000n, // Reasonable gas limit for expireVerificationMethod
        })
        ...
        throw new Error(
            `Failed to submit expireVerificationMethod raw transaction: ${
    ...
    // Parse VerificationMethodExpired event from the receipt
    const verificationMethodExpiredEvent = receipt.logs
        .map((log) => {
            try {
    ...
    if (!verificationMethodExpiredEvent) {
        throw new Error(`${EVENT_NAME} event not found in transaction receipt`)
    }

    const args = verificationMethodExpiredEvent.args
