...
const EVENT_NAME = 'ControllerRevoked'

export interface ControllerRevokedResult {
    did: string
    controller: string
}
...
export async function revokeController(
...
): Promise<ControllerRevokedResult> {
    console.log(
        `🔐 Using ${signatureProvider.getCurveType()} signature for revoking controller...`
    )

    if (signatureProvider.getCurveType() === 'secp256r1') {
        return await revokeControllerWithRawTransaction(
            hre,
            did,
            controller,
            diamond,
    ...
    console.log('📡 Sending revokeController transaction...')
    ...
    if (typeof args.did !== 'string' || typeof args.controller !== 'string') {
        throw new Error('Invalid ControllerRevoked event args format')
    }
    ...
    console.log(`\n✅ Controller revoked successfully:`)
...
/**
 * Revoke controller using raw transactions for secp256r1 compatibility
 * Avoids the "Cannot find square root" error by bypassing ethers Contract interface
 */
async function revokeControllerWithRawTransaction(
    hre: HardhatRuntimeEnvironment,
    did: string,
    controller: string,
    diamond: string,
    ...
    try {
        ...
        txResponse = await signatureProvider.sendTransaction({
            to: diamond,
            data: functionData,
            gasLimit: 500000n, // Reasonable gas limit for revokeController
        })
        ...
        throw new Error(
            `Failed to submit revokeController raw transaction: ${
    ...
    // Parse ControllerRevoked event from the receipt
    const controllerRevokedEvent = receipt.logs
        .map((log) => {
            try {
