...
// THIS CLASS will be moved to a common utils file later
class ErrorDecoder {
    ...
    constructor(
        ...
        contractName: string = 'AnchoringCoreFacet'
    ) {
        this.hre = hre
    ...
    async processTx(
        ...
        try {
            ...
            process.stdout.write(
                'Sending transaction to network. TX Hash: ' +
                    tx.hash +
                    '           \r'
            )
            ...
            if (error.data) {
                ...
                console.error(
                    'Decoded error:  \x1b[31m' + decodedError + '\x1b[0m \n\n'
                )
            }
            throw error
        }
    ...
    private async decodeError(data: string): Promise<string> {
        ...
        const abi = (await this.hre.artifacts.readArtifact(this.contractName))
            .abi
...
// THIS FUNCTION   be moved to a common utils file later
async function getContract(
    hre: HardhatRuntimeEnvironment,
    contractAddress: string
): Promise<Contract> {
    console.log(
        `Getting AnchoringCoreFacet contract at address: ${contractAddress}`
    )
...
export async function getRegisteredChains(
    ...
    console.log(
        `Using HRE default provider with network: ${await provider.getNetwork().then((n) => n.name)}`
    )
    ...
    console.log(
        `Fetching registered chains with pageIndex: ${pageIndexNum}, pageLength: ${pageLengthNum}`
    )
    const result = await connectedContract.getRegisteredChains(
        pageIndexNum,
        pageLengthNum
    )
    console.log(
        `Fetched ${result._registeredChainIds.length} registered chain IDs.`
    )
...
> {
    ...
    const result = await connectedContract.getBlocksInRange(
        chainid,
        startblock,
        endblock
    )
...
export async function getAnchoredBlock(
    ...
    const connectedContract = contract.connect(provider)
    const result = await connectedContract.getAnchoredBlock(
        chainid,
        blocknumber
    )
    return result
...
export async function registerChain(
    ...
    if (signatureProvider.getCurveType() === 'secp256r1') {
        ...
        const data = contract.interface.encodeFunctionData('registerChain', [
            chainIdNum,
        ])
        ...
        await errorDecoder.processTx(tx)
    } else {
        // For secp256k1, use standard contract call
        const connectedContract = contract.connect(signer)
        await errorDecoder.processTx(
            connectedContract.registerChain(chainIdNum)
        )
    }
...
export async function anchorBlock(
    ...
    if (signatureProvider.getCurveType() === 'secp256r1') {
        // For secp256r1, encode the function call and send as raw transaction
        const data = contract.interface.encodeFunctionData('anchorBlock', [
            chainIdNum,
            blockNumberNum,
            blockhash,
            stateroot,
...
export async function anchorBlocksBatch(
    ...
    blockNumbersNum.forEach((num, i) => {
        console.log(
            `    [${i}] blockNumber: ${num}, blockHash: ${blockHashesArr[i]}, stateRoot: ${stateRootsArr[i]}`
        )
    })
    ...
    if (signatureProvider.getCurveType() === 'secp256r1') {
        ...
        const connectedContract = contract.connect(signer)
        await errorDecoder.processTx(
            connectedContract.anchorBlocksBatch(
                chainIdNum,
                blockNumbersNum,
                blockHashesArr,
                stateRootsArr
