...
const EVENT_NAME = 'ResourceDeleted'
...
/**
 * Delete resource using signature provider pattern (supports both secp256k1 and secp256r1)
 */
export async function deleteResource(
...
): Promise<ResourceDeletedResult> {
    console.log(
        `🌐 Using ${signatureProvider.getCurveType()} signature for resource deletion...`
    )
    ...
    if (signatureProvider.getCurveType() === 'secp256r1') {
        return await deleteResourceWithRawTransaction(
            hre,
            normalizedChainId,
            resourceIdBytes32,
            diamond,
    ...
    console.log('📡 Sending deleteResource transaction...')
    let tx: ContractTransactionResponse
    try {
        tx = await networkDirectory.deleteResource(
            normalizedChainId,
            resourceIdBytes32
        )
    ...
    if (
        ...
        throw new Error('Invalid ResourceDeleted event args format')
    }

    const { chainId: evChainId, resourceId: evResourceId } = args
...
/**
 * Delete resource using raw transactions for secp256r1 compatibility
 * Avoids the "Cannot find square root" error by bypassing ethers Contract interface
 */
async function deleteResourceWithRawTransaction(
    hre: HardhatRuntimeEnvironment,
    chainId: bigint,
    ...
    // Encode the deleteResource function call
    const functionData = contractInterface.encodeFunctionData(
        'deleteResource',
        [chainId, resourceIdBytes32]
    )

    console.log('📡 Sending deleteResource raw transaction...')
    ...
    try {
        ...
        txResponse = await signatureProvider.sendTransaction({
            to: diamond,
            data: functionData,
            gasLimit: 200000n, // Reasonable gas limit for deleteResource
        })
        ...
        throw new Error(
            `Failed to submit deleteResource raw transaction: ${
    ...
    // Parse ResourceDeleted event from the receipt
    const resourceDeletedEvent = receipt.logs
        .map((log) => {
            try {
