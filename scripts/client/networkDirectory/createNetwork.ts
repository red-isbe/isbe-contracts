...
const EVENT_NAME = 'NetworkCreated'
...
export interface NetworkCreatedResult {
    chainId: bigint
    name: string
    symbol: string
}
...
export async function createNetwork(
    ...
    if (
        ...
        throw new Error('Invalid NetworkCreated event args format')
    }

    const { chainId: evChainId, name: evName, symbol: evSymbol } = eventNetwork

    if (evChainId !== BigInt(network.chainId)) {
        console.warn(
...
async function createNetworkWithRawTransaction(
    ...
    // Encode the createNetwork function call
    const functionData = contractInterface.encodeFunctionData('createNetwork', [
        networkTuple,
    ])
    ...
    try {
        ...
        throw new Error(
            `Failed to submit createNetwork raw transaction: ${
    ...
    const networkCreatedEvent = receipt.logs
        .map((log) => {
            try {
    ...
    const args = networkCreatedEvent.args
    const eventNetwork = args.network
