...
const EVENT_NAME = 'BootNodeRemoved'

export async function removeBootNode(
    ...
    console.log(
        `🔐 Using ${signatureProvider.getCurveType()} signature for boot node removal...`
    )

    if (signatureProvider.getCurveType() === 'secp256r1') {
        return await removeBootNodeWithRawTransaction(
            hre,
            nodeId,
            diamond,
            signatureProvider
    ...
    console.log('📡 Sending removeBootNode transaction...')
    ...
    if (typeof args.nodeId !== 'string') {
        throw new Error('Invalid BootNodeRemoved event args format')
    }
...
/**
 * Remove boot node using raw transactions for secp256r1 compatibility
 * Avoids the "Cannot find square root" error by bypassing ethers Contract interface
 */
async function removeBootNodeWithRawTransaction(
    hre: HardhatRuntimeEnvironment,
    nodeId: string,
    diamond: string,
    signatureProvider: ISignatureProvider
    ...
    // Encode the removeBootNode function call
    ...
    try {
        ...
        txResponse = await signatureProvider.sendTransaction({
            to: diamond,
            data: functionData,
            gasLimit: 200000n, // Reasonable gas limit for removeBootNode
        })
        ...
        throw new Error(
            `Failed to submit removeBootNode raw transaction: ${
