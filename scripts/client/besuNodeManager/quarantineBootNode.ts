...
const QUARANTINE_EVENT_NAME = 'BootNodeQuarantined'

export async function quarantineBootNode(
    ...
    console.log(
        `🔐 Using ${signatureProvider.getCurveType()} signature for boot node quarantine...`
    )

    if (signatureProvider.getCurveType() === 'secp256r1') {
        return await quarantineBootNodeWithRawTransaction(
            hre,
            nodeId,
            diamond,
            signatureProvider
    ...
    console.log('📡 Sending quarantineBootNode transaction...')
    let tx: ContractTransactionResponse
    try {
        tx = await besuNodeManager.quarantineBootNode(nodeId)
    ...
    if (typeof args.nodeId !== 'string') {
        throw new Error('Invalid BootNodeQuarantined event args format')
    }
...
/**
 * Quarantine boot node using raw transactions for secp256r1 compatibility
 * Avoids the "Cannot find square root" error by bypassing ethers Contract interface
 */
async function quarantineBootNodeWithRawTransaction(
    hre: HardhatRuntimeEnvironment,
    nodeId: string,
    diamond: string,
    signatureProvider: ISignatureProvider
    ...
    // Encode the quarantineBootNode function call
    const functionData = contractInterface.encodeFunctionData(
        'quarantineBootNode',
        [nodeId]
    )

    console.log('📡 Sending quarantineBootNode raw transaction...')
    ...
    try {
        ...
        txResponse = await signatureProvider.sendTransaction({
            to: diamond,
            data: functionData,
            gasLimit: 200000n, // Reasonable gas limit for quarantineBootNode
        })
        ...
        throw new Error(
            `Failed to submit quarantineBootNode raw transaction: ${
    ...
    // Parse BootNodeQuarantined event from the receipt
    const bootNodeQuarantinedEvent = receipt.logs
        .map((log) => {
            try {
    ...
    if (!bootNodeQuarantinedEvent) {
        throw new Error(
            `${QUARANTINE_EVENT_NAME} event not found in transaction receipt`
        )
    }

    const args = bootNodeQuarantinedEvent.args
