...
const PROMOTE_EVENT_NAME = 'ValidatorPromoted'

export async function promoteValidator(
    ...
    console.log(
        `🔐 Using ${signatureProvider.getCurveType()} signature for validator promotion...`
    )

    if (signatureProvider.getCurveType() === 'secp256r1') {
        return await promoteValidatorWithRawTransaction(
            hre,
            nodeId,
            diamond,
            signatureProvider
    ...
    console.log('📡 Sending promoteValidator transaction...')
    let tx: ContractTransactionResponse
    try {
        tx = await besuNodeManager.promoteValidator(nodeId)
    ...
    if (!logDescription) {
        throw new Error(
            `${PROMOTE_EVENT_NAME} event not found in transaction logs`
        )
    }
    ...
    if (typeof args.nodeId !== 'string') {
        throw new Error('Invalid ValidatorPromoted event args format')
    }
...
/**
 * Promote validator using raw transactions for secp256r1 compatibility
 * Avoids the "Cannot find square root" error by bypassing ethers Contract interface
 */
async function promoteValidatorWithRawTransaction(
    hre: HardhatRuntimeEnvironment,
    nodeId: string,
    diamond: string,
    signatureProvider: ISignatureProvider
    ...
    // Encode the promoteValidator function call
    const functionData = contractInterface.encodeFunctionData(
        'promoteValidator',
        [nodeId]
    )

    console.log('📡 Sending promoteValidator raw transaction...')
    ...
    try {
        ...
        throw new Error(
            `Failed to submit promoteValidator raw transaction: ${
    ...
    // Parse ValidatorPromoted event from the receipt
    const validatorPromotedEvent = receipt.logs
        .map((log: any) => {
            try {
        ...
        .find((log: any) => log && log.name === PROMOTE_EVENT_NAME)

    if (!validatorPromotedEvent) {
        throw new Error(
            `${PROMOTE_EVENT_NAME} event not found in transaction receipt`
        )
    }

    const args = validatorPromotedEvent.args
