...
const QUARANTINE_EVENT_NAME = 'ExecutionNodeQuarantined'

export async function quarantineExecutionNode(
    ...
    console.log(
        `🔐 Using ${signatureProvider.getCurveType()} signature for execution node quarantine...`
    )

    if (signatureProvider.getCurveType() === 'secp256r1') {
        return await quarantineExecutionNodeWithRawTransaction(
            hre,
            nodeId,
            diamond,
            signatureProvider
    ...
    console.log('📡 Sending quarantineExecutionNode transaction...')
    ...
    if (typeof args.nodeId !== 'string') {
        throw new Error('Invalid ExecutionNodeQuarantined event args format')
    }
...
/**
 * Quarantine execution node using raw transactions for secp256r1 compatibility
 * Avoids the "Cannot find square root" error by bypassing ethers Contract interface
 */
async function quarantineExecutionNodeWithRawTransaction(
    hre: HardhatRuntimeEnvironment,
    nodeId: string,
    diamond: string,
    signatureProvider: ISignatureProvider
    ...
    // Encode the quarantineExecutionNode function call
    ...
    try {
        ...
        txResponse = await signatureProvider.sendTransaction({
            to: diamond,
            data: functionData,
            gasLimit: 200000n, // Reasonable gas limit for quarantineExecutionNode
        })
        ...
        throw new Error(
            `Failed to submit quarantineExecutionNode raw transaction: ${
    ...
    const args = executionNodeQuarantinedEvent.args
