...
export async function addValidatorStandby(
    ...
    if (signatureProvider.getCurveType() === 'secp256r1') {
        return await addValidatorStandbyWithRawTransaction(
            hre,
            enode,
            diamond,
            signatureProvider
    ...
    console.log('📡 Sending addValidatorStandby transaction...')
    let tx: ContractTransactionResponse
    try {
        tx = await besuNodeManager.addValidatorStandby(enode)
...
/**
 * Add validator in standby state using raw transactions for secp256r1 compatibility
 * Avoids the "Cannot find square root" error by bypassing ethers Contract interface
 */
async function addValidatorStandbyWithRawTransaction(
    hre: HardhatRuntimeEnvironment,
    enode: string,
    diamond: string,
    signatureProvider: ISignatureProvider
