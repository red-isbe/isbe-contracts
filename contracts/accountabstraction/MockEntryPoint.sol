    ...
    EIP712
{
    ...
    uint256 private constant _INNER_GAS_OVERHEAD = 10000;

    // Marker for inner call revert on out of gas
    bytes32 private constant _INNER_OUT_OF_GAS = hex'deaddead';
    bytes32 private constant _INNER_REVERT_LOW_PREFUND = hex'deadaa51';

    uint256 private constant _REVERT_REASON_MAX_LEN = 2048;
    // Penalty charged for either unused execution gas or postOp gas
    uint256 private constant _UNUSED_GAS_PENALTY_PERCENT = 10;
    // Threshold below which no penalty would be charged
    uint256 private constant _PENALTY_GAS_THRESHOLD = 40000;

    string internal constant _DOMAIN_NAME = 'ERC4337';
    string internal constant _DOMAIN_VERSION = '1';

    SenderCreator private immutable _SENDER_CREATOR = new SenderCreator();

    constructor() EIP712(_DOMAIN_NAME, _DOMAIN_VERSION) {}

    /**
     * @notice Custom method to ease Paymaster validation as only EntryPoint can
     *          call this method.
     * @param paymaster The paymaster that handles the call.
    ...
    function innerHandleOp(
        ...
        if (callData.length > 0) {
            ...
            if (!success) {
                ...
                bytes memory result = Exec.getReturnData(
                    _REVERT_REASON_MAX_LEN
                );
                if (result.length > 0) {
    ...
    function getUserOpHash(
        ...
        bytes32 overrideInitCodeHash = hex'';
        return
    ...
    function _executeUserOp(
        ...
        if (!success) {
            ...
            if (innerRevertCode == _INNER_OUT_OF_GAS) {
                // handleOps was called with gas limit too low. abort entire bundle.
                // can only be caused by bundler (leaving not enough gas for inner call)
                revert FailedOp(opIndex, 'AA95 out of gas');
            } else if (innerRevertCode == _INNER_REVERT_LOW_PREFUND) {
                ...
                emit PostOpRevertReason(
                    ...
                    Exec.getReturnData(_REVERT_REASON_MAX_LEN)
                );
                _restoreFreePtr(freePtr);
        ...
        unchecked {
            ...
            if (paymaster == address(0)) {
                if (!_tryDecrementDeposit(sender, requiredPrefund)) {
                    revert FailedOp(opIndex, 'AA21 did not pay prefund');
                }
            }
        }
    }
    ...
    function _callValidatePaymasterUserOp(
        ...
        unchecked {
            if (
                !success ||
                contextOffset != 64 ||
                contextLength + 31 < maxContextLength
            ) {
                revert FailedOpWithRevert(
                    opIndex,
                    'AA33 reverted',
        ...
        unchecked {
            ...
            if (prefund < actualGasCost) {
                if (mode == IPaymaster.PostOpMode.postOpReverted) {
                    ...
                    assembly ('memory-safe') {
                        mstore(0, _INNER_REVERT_LOW_PREFUND)
                        revert(0, 32)
                    }
                }
            } else {
    ...
    function _validateAccountAndPaymasterValidationData(
        ...
        // Non-zero address means that the paymaster fails due to some signature check
        // (which is ok only during estimation).
    ...
    function _getValidationData(
        uint256 validationData
    ) internal view virtual returns (address aggregator, bool outOfTimeRange) {
        if (validationData == 0) {
    ...
    function _getUserOpGasPrice(
        ...
        unchecked {
            ...
            return min(maxFeePerGas, maxPriorityFeePerGas + 0);
        }
    }
     ...
     * NOTE: all dynamic allocations between saveFreePtr and restoreFreePtr MUST NOT be used
     *       after restoreFreePtr is called.
     */
    ...
    function _getUnusedGasPenalty(
        ...
        unchecked {
            if (gasLimit <= gasUsed + _PENALTY_GAS_THRESHOLD) {
                return 0;
            }
            uint256 unusedGas = gasLimit - gasUsed;
            uint256 unusedGasPenalty = (unusedGas *
                _UNUSED_GAS_PENALTY_PERCENT) / 100;
