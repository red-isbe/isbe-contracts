...
abstract contract ERC712Internal {
    ...
    function _initialize(string memory _name, string memory _version) internal {
        ERC712Storage storage storage_ = _erc712Storage();
        storage_.name = _name;
        storage_.version = _version;
        storage_.cachedChainId = block.chainid;
        storage_.cachedDomainSeparator = _buildDomainSeparator();
    }
    ...
    function _verifyBurnSignature(
        ...
        require(
            signer != address(0) && signer == from,
            IERC712.InvalidSignature()
        );
    }
    ...
    function _useNonce(address owner) internal returns (uint256 current) {
        ERC712Storage storage storage_ = _erc712Storage();
        current = storage_.nonces[owner];
        unchecked {
            storage_.nonces[owner] = current + 1;
        }
    }
    ...
    function _domainSeparatorV4() internal view returns (bytes32) {
        ...
        if (block.chainid == storage_.cachedChainId) {
            return storage_.cachedDomainSeparator;
        } else {
    ...
    function _buildDomainSeparator() internal view returns (bytes32) {
        ...
        return
            keccak256(
                abi.encode(
                    _ERC712_TYPE_HASH,
                    keccak256(bytes(storage_.name)),
                    keccak256(bytes(storage_.version)),
                    block.chainid,
                    address(this)
                )
            );
     ...
     * @return storage_ The storage layout at the defined storage position
     */
    function _erc712Storage()
        internal
        pure
        returns (ERC712Storage storage storage_)
    {
