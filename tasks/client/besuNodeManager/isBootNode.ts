...
import { isBootNode } from '../../../scripts/client/besuNodeManager/isBootNode'

/**
 * Gets status of a node.
 npx hardhat isBootNode --network dev \
 ...
 */
task('isBootNode', 'Retrieves state of a BootNode from the BesuNodeManager.')
    .addParam(
    ...
    .setAction(
        async (
            ...
            const BootNode = await isBootNode(
                besuNodeManagerAddress,
                nodeId,
                signer
            )
            console.log('Is a BootNode: ', BootNode)
        }
    )
