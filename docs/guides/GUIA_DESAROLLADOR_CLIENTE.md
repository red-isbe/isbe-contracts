...
1.  **`IMiFuncionalidad.sol`**: La interfaz pública (eventos, errores, structs, firmas de funciones).
2.  **`MiFuncionalidadInternal.sol`**: Gestión del almacenamiento (struct Storage + accessor assembly) y funciones internas.
3.  **`MiFuncionalidad.sol`**: Abstract Core - Implementación pública con control de acceso (roles, modifiers).
4.  **`MiFuncionalidadFacet.sol`**: Facet desplegable - Punto de entrada para el Diamond (introspección EIP-2535).
...
### 3. Abstract Core (`MiFuncionalidad.sol`)

Implementa la interfaz pública con control de acceso. Este es el contrato abstracto que conecta la lógica interna con las funciones externas.
