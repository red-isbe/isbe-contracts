...
3. Entry Point
    1. EntryPointInternal - implements required business logic to support entry point functionality. All methods are internal. It uses functionality from `NonceManagerInternal` and `StakeManagerInternal`.
    2. EntryPoint - implements [IEntryPoint](https://github.com/eth-infinitism/account-abstraction/blob/develop/contracts/interfaces/IEntryPoint.sol), [IStakeManager](http://github.com/eth-infinitism/account-abstraction/blob/develop/contracts/interfaces/IStakeManager.sol) and [INonceManager](https://github.com/eth-infinitism/account-abstraction/blob/develop/contracts/interfaces/INonceManager.sol) interfaces from the standard. It exposes external functions.
