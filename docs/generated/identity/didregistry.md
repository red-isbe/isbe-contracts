...
_Performs O(1) lookup and validates:
     - Address is mapped to a DID
     - Verification method exists and is not revoked
     - Capability invocation relationship exists and is temporally valid_
...
```solidity
function _validateProof(bytes32 _did, bytes _proof, bytes _publicKey) internal pure
```
...
| Name | Type | Description |
| ---- | ---- | ----------- |
| _did | bytes32 | The decentralised identifier to validate (must have 13 zero prefix + 19 payload) |
| _proof | bytes | The signature proving ownership (65 bytes for ECDSA: r:32 + s:32 + v:1) |
