...
Esto asegura que el código auditado es _exactamente_ el código que se ejecuta en la red, eliminando el riesgo de que un cliente despliegue una versión modificada post-auditoría.
...
- **Validación:** `selectorsIntrospection()` debe retornar _todos_ y _solo_ los selectores públicos expuestos.
...
2.  **El cliente firma y ejecuta la transacción `diamondCut`** siguiendo la _Guía de Desarrollador_.
