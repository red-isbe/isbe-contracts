...
interface INetworkCatalog {
    ...
    // Recursos
    function setResource(uint256 chainId, bytes32 key, string calldata resource) external;
    function deleteResource(uint256 chainId, bytes32 key) external;
    function listResourceKeys(uint256 chainId) external view returns (bytes32[] memory);
}
```
